    ...
    <div class="visualizador-modal" @click.stop>
      ...
      <div class="modal-header">
        <div class="header-info">
          <h2 class="modal-title">{{ documento?.nome || 'Visualizar arquivo' }}</h2>
      ...
      <div class="visualizador-content">
        ...
        <div v-else-if="erro" class="error-container">
          ...
          <p>{{ erro }}</p>
          <button class="retry-button" @click="carregarArquivo">Tentar novamente</button>
        </div>
...
const tipoArquivo = computed(() => {
  if (!props.documento?.url) return ''
  const url = props.documento.url
...
const carregarArquivo = async () => {
  if (!props.documento?.url) {
    erro.value = 'URL do arquivo não encontrada'
    return
  }
  ...
  try {
    ...
    if (props.documento.url.includes('supabase')) {
      // Extrair path do arquivo da URL
      const urlObj = new URL(props.documento.url)
      ...
      urlArquivo.value = props.documento.url
    }
    
  } catch (error) {
    console.error('Erro ao carregar arquivo:', error)
    erro.value = 'Não foi possível carregar o arquivo. Verifique se ele ainda existe.'
  } finally {
...
const baixarArquivo = () => {
  if (urlArquivo.value) {
    ...
    link.download = props.documento?.nome || 'arquivo'
    link.target = '_blank'
