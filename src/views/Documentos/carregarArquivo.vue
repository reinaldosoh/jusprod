...
const opcoesCategoria = ref([
  { id: 1, label: 'Documento Geral' },
  { id: 2, label: 'Contrato' },
  { id: 3, label: 'Procuração' },
  { id: 4, label: 'Petição' },
  { id: 5, label: 'Parecer' }
])
...
const cancelar = () => {
  if (titulo.value.trim() || arquivo.value) {
    if (confirm('Tem certeza que deseja cancelar? O arquivo não será salvo.')) {
      fechar()
    }
  } else {
    fechar()
...
const salvar = async () => {
  ...
  try {
    ...
    // Bucket 'documentos' já existe e está configurado
    ...
    console.log('Fazendo upload do arquivo:', nomeArquivo)
    ...
    // Emitir sucesso
    console.log('✅ Arquivo carregado com sucesso:', data[0])
    emit('arquivo-carregado', data[0])
    fechar()
    
  } catch (error) {
    console.error('Erro ao carregar arquivo:', error)
    emit('erro', 'Erro ao carregar arquivo: ' + error.message)
  } finally {
