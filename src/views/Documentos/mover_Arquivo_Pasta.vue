<template>
  <div class="modal-overlay" @click.self="$emit('close')">
    <div class="modal-content">
      ...
      <!-- Título -->
      <h2 class="titulo">Mover documento</h2>

      <!-- Subtítulo -->
      <p class="subtitulo">Selecione a pasta de destino para "{{ documento?.nome }}"</p>
...
const emit = defineEmits(['close', 'documento-movido'])
...
async function carregarPastas() {
  try {
    ...
    const { data, error } = await supabase
      ...
      .neq('id', props.documento.pasta_id) // Excluir pasta atual
...
// Função para mover o documento para a pasta selecionada
async function moverDocumento() {
  ...
  try {
    ...
    // Atualizar documento com nova pasta_id
    ...
    alert('Erro ao mover o documento. Tente novamente.')
  } finally {
    loading.value = false
  }
}
